// Background de la extensión Confirmo (variante Firefox: background.scripts,
// no service_worker -- ver manifest.json de esta carpeta).
...
// Firefox no implementa chrome.sidePanel (API exclusiva de Chrome/Edge) --
// usa su propio modelo de sidebar (browser.sidebarAction), que además no
// tiene equivalente de "abrir automáticamente al hacer clic en el ícono"
// (sidePanel.setPanelBehavior en Chrome); por eso acá se agrega un listener
// chrome.action.onClicked que llama a browser.sidebarAction.open() a mano.
// El resto (chrome.runtime, chrome.tabs, chrome.scripting) funciona igual en
// ambos navegadores vía el alias chrome.* que Firefox provee -- ver también
// AppExtension/background.js, que usa el mismo patrón.
...
if (chrome.sidePanel?.setPanelBehavior) {
  ...
  chrome.action.onClicked.addListener(async () => {
    try {
      ...
      console.warn("No se pudo abrir el sidebar de Confirmo (Firefox):", error);
    }
  });
}
